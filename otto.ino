...
#define FR       5  // Pe dereito (Right Foot)
#define LR       3  // Perna dereita (Right Leg)
#define FL       4  // Pe esquerdo (Left Foot)
...
void setup() {
 ...
 void loop() {      //non escribimos nada de codigo dentro do loop como indica o enunciado
  manolitogay();    //declaramos as funcions que entran no loop
}


 bool manolitogay()  //valor binario para la funcion
{
 bool objeto;
  if (distanciaObxecto() < UMBRAL_DIST) //si la distancia es menor al umbral suena
  { 
    zumbar();
    objeto = true;  
  }
  else     //si esta no es inferior al umbral (mas de 12cm) no suena
  {
    nonZumbar();
    objeto = false;
   delay(50);
...
void zumbar() {
    digitalWrite(BUZZER, HIGH);    //a funcion xa esta feita pero escribimos o codigo que o permite sonar
	delay(20);
...
void nonZumbar() {
    digitalWrite(BUZZER, LOW);    //a funcion xa esta feita pero escribimos o codigo que lle impide sonar
	delay(20);
